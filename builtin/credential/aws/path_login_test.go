...
func TestBackend_validateVaultHeaderValue(t *testing.T) {
	...
	err = validateVaultHeaderValue("POST", postHeadersMissing, requestURL, canaryHeaderValue)
	if err == nil {
		t.Error("validated POST request with missing Vault header")
	}

	err = validateVaultHeaderValue("POST", postHeadersInvalid, requestURL, canaryHeaderValue)
	if err == nil {
		t.Error("validated POST request with invalid Vault header value")
	}

	err = validateVaultHeaderValue("POST", postHeadersUnsigned, requestURL, canaryHeaderValue)
	if err == nil {
		t.Error("validated POST request with unsigned Vault header")
	}

	err = validateVaultHeaderValue("POST", postHeadersValid, requestURL, canaryHeaderValue)
	if err != nil {
		t.Errorf("did NOT validate valid POST request: %v", err)
	}

	err = validateVaultHeaderValue("POST", postHeadersSplit, requestURL, canaryHeaderValue)
	if err != nil {

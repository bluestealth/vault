...
func TestBackend_validateVaultGetRequestValues(t *testing.T) {
	...
	err = validateVaultHeaderValue("GET", getHeadersValid, noActionGetRequestURL, canaryHeaderValue)
	if err == nil {
		t.Error("validated GET request with no Action parameter")
	}

	err = validateVaultHeaderValue("GET", getHeadersValid, invalidActionGetRequestURL, canaryHeaderValue)
	if err == nil {
